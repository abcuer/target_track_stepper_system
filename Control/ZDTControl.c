...
void usart3_SendCmd(uint8_t *buf, uint16_t len)
{
    for (uint16_t i = 0; i < len; i++)
    {
...
/**
  * @brief    控制电机转动到指定角度（角度模式封装）
  * @param    addr       电机地址
  * @param    angle_deg  目标角度（单位：度，正为CW，负为CCW）
  * @param    vel_rpm    转速（单位：RPM，范围 0 ~ 5000）
  * @param    acc        加速度（0 为瞬时启动，最大 255）
  * @param    abs_mode   是否为绝对位置模式（true：绝对位置，false：相对位置）
  */
...
void Emm_V5_Move_To_Angle(uint8_t addr, float angle_deg, uint16_t vel_rpm, uint8_t acc, bool abs_mode)
{
    ...
    clk = (uint32_t)(angle_deg * 51200.0f / 360.0f);
...
void Emm_V5_Pos_Control(uint8_t addr, uint8_t dir, uint16_t vel, uint8_t acc, uint32_t clk, bool raF, bool snF)
{
  ...
  // 发送命令
  if(addr == 1) usart_SendCmd(cmd, 13);
  else if(addr == 2) usart3_SendCmd(cmd, 13);
}
